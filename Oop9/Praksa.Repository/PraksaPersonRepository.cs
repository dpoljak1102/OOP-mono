...
namespace Praksa.Repository
{
    public class PraksaPersonRepository : IPraksaPersonRepository
    {
        ...
        public  async Task<List<Person>> GetAllPeopleAsync(FilterName urlQuery)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                if (urlQuery.HaveFilter)
                {
                    ...
                    {
                        additionQueryString += $"First_Name = '{urlQuery.FirstName}'";

                    }
                    if (!string.IsNullOrEmpty(urlQuery.LastName))
                    {
                        if (!string.IsNullOrEmpty(additionQueryString))
                        {
                            additionQueryString += " AND ";
                        }
                        additionQueryString += $"Last_Name = '{urlQuery.LastName}'";
                    }
                    queryString = $"SELECT * FROM Person WHERE {additionQueryString}";
                }
