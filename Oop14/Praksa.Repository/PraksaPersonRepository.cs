...
namespace Praksa.Repository
{
    public class PraksaPersonRepository : IPraksaPersonRepository
    {
        ...
        //FILTER, PAGE, SORT
        public  async Task<List<Person>> GetAllPeopleAsync(Filters filters, Pages pages, Sorts sorts)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                //If we don't have search(filter,sorts)
                //string defaultQueryString = $"SELECT TOP {page.PageSize} * FROM Person ";
                string queryString = "";
                if (filters == null || pages == null)
                {
                    queryString = $"SELECT TOP {pages.PageSize} * FROM Person ";
                }
                else
                {
                    queryString = $"SELECT TOP {pages.PageSize} * FROM Person " +
                                 ...
                                 $"ORDER BY  {sorts.OrderBy} {sorts.AscDesc}";
